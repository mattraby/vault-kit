...
This file is the **schema layer** of the vault: it defines how knowledge is structured and
maintained. It is configuration, not a concept page — it is exempt from the OKF frontmatter
rules below (like `index.md` and `log.md`). Read it before adding or changing anything.
...
```
vault/
├── CLAUDE.md          # this schema (config — exempt from frontmatter rules)
...
1. **Read** the source and discuss takeaways with the human.
...
Scan for and report: contradictions between pages, stale `timestamp`s, orphaned pages (no
inbound links), missing cross-links, thin/empty pages, and **OKF conformance** — every
non-reserved `.md` (excluding `CLAUDE.md`, `index.md`, `log.md`) must have non-empty `type`
